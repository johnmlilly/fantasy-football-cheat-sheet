...
import { Star, Trash2, Plus, Printer, LogOut } from "lucide-react";
        ...
        <div className="bg-white border border-stone-200 print:border-none rounded-b-lg print:rounded-none divide-y divide-stone-100">
          ...
          {groups.map((group) => (
            <div key={group.label ?? "all"}>
              ...
              {group.items.map((p) => (
                <div key={p.id} className="px-4 sm:px-6 py-2 flex items-center gap-2 sm:gap-3 flex-wrap hover:bg-stone-50 print:py-1 print:break-inside-avoid">
                  <button onClick={() => toggleStar(p.id)} className="shrink-0 print:hidden">
                  ...
                  <button onClick={() => deletePlayer(p.id)} className="shrink-0 text-stone-300 hover:text-red-500 print:hidden">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
